...
(function($){
	if ($.validator) {
		$.validator.addMethod("chejiaNum", function(value, element) {
			return this.optional(element) || /^(?!(?:\d+|[a-zA-Z]+)$)[\da-zA-Z]{17}$/ .test(value);
		}, "请输入一个有效的车架号");
		$.validator.addMethod("carNum", function(value, element) {
		...
		$.validator.addMethod("phone", function(value, element) {
			//return this.optional(element) || /^[0-9 \(\)]{7,30}$/.test(value);
			return this.optional(element) || /(^(\d{3,4}-)?\d{7,8})$|(1\d{10})$/ .test(value);
		}, "请输入一个有效的手机号码");
...
function initUI(_box){
	...
	$("form.required-validate1", $p).each(function(){
		var $form = $(this);
		$form.validate({
			onsubmit: false,
			focusInvalid: false,
