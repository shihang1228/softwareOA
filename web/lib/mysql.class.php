...
class mysql
{
	...
	function connect($host = "", $user = "", $pwd = "", $dbName = "")
	{
		...
		$this->linkID = mysql_connect($host, $user, $pwd);//连接数据库
		if (!$this->linkID)
		{
			$this->halt();/*连接失败时抛出异常，告诉你发生错误的文件名，第几行，给程序员看的。
			               $this->error是给用户看的，比如登陆时密码不正确就用$this->error*/
			return 0;
		}
		if (!mysql_select_db($dbName, $this->linkID))//数据库与连接建立联系
		{
			$this->halt();
			return 0;
		}
	...
	{
		$this->queryTimes++;
		$this->queryID = mysql_query($sql, $this->linkID);//将sql语句给MySQL执行，增删改查都是这句
		if (!$this->queryID)
		{	
			$this->halt();
			return 0;
	...
	{
		return mysql_num_rows($this->queryID);//返回记录数
	}
	//======================================
	// 函数: getQueryTimes()
	// 功能: 返回查询的次数
